...
class UXAuditor:
    ...
    def audit_file(self, filepath: str) -> None:
        ...
        # --- 1. PSYCHOLOGY LAWS ---
        # Hick's Law - Modified for specific nav context
        nav_content = re.search(r'<nav[^>]*>.*?</nav>', content, re.DOTALL | re.IGNORECASE)
        nav_items = 0
        if nav_content:
            nav_items = len(re.findall(r'<a\s+href', nav_content.group(0), re.IGNORECASE))
        ...
        # Familiar patterns - Restricted to HTML-like files and only if inputs actually exist
        is_html_like = filename.lower().endswith(('.html', '.tsx', '.jsx', '.vue', '.svelte'))
        has_actual_inputs = bool(re.search(r'<input|<select|<textarea', content, re.IGNORECASE))
        if has_form and is_html_like and has_actual_inputs:
