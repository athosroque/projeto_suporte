...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateChart() {
        const sysCtx = document.getElementById('supportChart');
        const prioCtx = document.getElementById('priorityChart');
        if (!sysCtx || !prioCtx) return;

        // System Data
        const systemCounts = localTickets.reduce((acc, t) => {
            const system = t.system; // Usar o nome completo para evitar problemas de agrupamento
            acc[system] = (acc[system] || 0) + 1;
            return acc;
        }, {});

        // Priority Data
        const priorityCounts = localTickets.reduce((acc, t) => {
            acc[t.priority] = (acc[t.priority] || 0) + 1;
            return acc;
        }, {});

        const sysLabels = Object.keys(systemCounts);
        const sysData = Object.values(systemCounts);
        const prioLabels = Object.keys(priorityCounts);
        const prioData = Object.values(priorityCounts);

        const colorMap = {
            'Crítico': 'rgba(255, 59, 48, 0.8)',
            'Alto': 'rgba(255, 149, 0, 0.8)',
            'Médio': 'rgba(255, 204, 0, 0.8)',
            'Baixo': 'rgba(52, 199, 89, 0.8)'
        };

        const prioColors = prioLabels.map(label => colorMap[label] || 'rgba(100, 100, 100, 0.8)');

        // Support Chart (Bar)
        if (supportChart) {
            supportChart.data.labels = sysLabels;
            supportChart.data.datasets[0].data = sysData;
            supportChart.update();
        } else {
            supportChart = new Chart(sysCtx, {
                type: 'bar',
                data: {
                    labels: sysLabels,
                    datasets: [{
                ...
                options: {
                    ...
                    scales: {
                        y: { beginAtZero: true, grid: { color: 'rgba(255,255,255,0.05)' }, ticks: { color: '#A1A1A6' } },
        ...
        // Priority Chart (Pie)
        if (priorityChart) {
            priorityChart.data.labels = prioLabels;
            priorityChart.data.datasets[0].data = prioData;
            ...
            priorityChart = new Chart(prioCtx, {
                ...
                options: {
                    ...
                    plugins: {
                        legend: {
                            position: 'bottom',
                            labels: { color: '#A1A1A6', font: { size: 10 }, boxWidth: 10 }
                        },
                        tooltip: {
                            callbacks: {
    ...
    function initializeLogs() {
        ...
        const initialLogs = [
            { level: 'INFO', service: 'System', message: 'SuporteData inicializado com sucesso' },
    ...
    function classifyTicket(description, user) {
        ...
        } else if (text.includes('relatório') || text.includes('dados') || text.includes('exportar') || text.includes('dashboard') || /\b(bi|b\.i)\b/.test(text)) {
            category = 'B.I.';
        ...
        if (text.includes('portal') && text.includes('família')) {
            ...
            system = 'App Mobile';
        } else if (text.includes('api') || text.includes('integração') || text.includes('webhook')) {
        ...
        } else if (text.includes('dashboard') || /\b(bi|b\.i)\b/.test(text) || text.includes('gráfico') || text.includes('métrica') || text.includes('analytics')) {
            system = 'BI & Analytics';
