...
function parseSimpleSQL(query) {
    ...
    // Mapeamento de status amigáveis para internos
    const statusMap = {
        'aberto': 'aberto',
        'em analise': 'analise',
        'analise': 'analise',
    ...
    if (upperQuery.includes('WHERE')) {
        ...
        if (whereMatch) {
            ...
            results = results.filter(t => {
                ...
                let searchVal = normalize(value);
                let targetVal = normalize(fieldValue);

                // Tratamento especial para status
                if (field === 'status') {
                    searchVal = statusMap[searchVal] || searchVal;
                }

                return targetVal.includes(searchVal) || searchVal.includes(targetVal);
            });
        }
    }
